...
## ⚫ Norm (norminette) — 62 hata kaldı (48 dosyada; 42 header hariç sadece
   `TOO_MANY_LINES` (12) ve `TOO_MANY_VARS_FUNC` (2) kaldı — taze tarama: 2026-07-09)
...
- [x] **42 header ve `TOO_MANY_LINES`/`TOO_MANY_VARS_FUNC` hariç kalan TÜM norm
      hataları** — **DÜZELTİLDİ** (190 → 62 hata; kalan 62'nin 48'i
      `INVALID_HEADER`, 12'si `TOO_MANY_LINES`, 2'si `TOO_MANY_VARS_FUNC` —
      bunlar bilinçli olarak dokunulmadı, kullanıcı isteği böyleydi).
      Düzeltilen kategoriler ve nasıl: `WRONG_SCOPE_COMMENT` (33, hepsi
      fonksiyon gövdesi içi satır-arası Türkçe açıklama yorumları — silindi;
      `exec_path.c` bunların çoğunu eski `exec_cmd.c`'den miras almıştı),
      `LINE_TOO_LONG` (26, uzun satırlar `||`/`&&` öncesi satır başına
      bölünerek kısaltıldı), `EMPTY_LINE_FUNCTION` (20, fonksiyon içi boş
      satırlar silindi), `BRACE_SHOULD_EOL` (11 — hepsi dosya sonunda eksik
      newline'dı, `builtins.c/ft_cd.c/ft_echo.c/ft_env.c/ft_exit.c/ft_pwd.c/
      ft_unset.c/exec_cmd.c/exec_pipe.c/exec_redir.c/executor.c`), 
      `MISALIGNED_FUNC_DECL` (9, `executor.h`+`utils.h` — dosyadaki İLK
      prototipin tab hizası tüm dosya için referans oluşturuyor, sonraki
      prototipler ona uymalı), `CONSECUTIVE_NEWLINES` (5), `INCLUDE_START_FILE`
      (4 — `minishell.h`'deki enum/struct tanımları `types.h` adlı yeni bir
      dosyaya taşındı ki tüm `#include`'lar dosyanın en başında, kod
      satırlarından önce olabilsin; `tokenizer.h`/`parser.h`/`expand.h` artık
      `t_type`/`t_token` gibi tipler tanımlandıktan hemen sonra, en üstte
      include ediliyor), `EOL_OPERATOR` (4, `||`/`&&` satır sonundan satır
      başına taşındı), `TERNARY_FBIDDEN` (3, `expand_append.c`'de if/else'e
      çevrildi), `SPACE_AFTER_KW` (3, `return;`→`return ;`, `continue;`→
      `continue ;`), `DECL_ASSIGN_LINE` (3, `test_tokenizer.c`'de dizi
      initializer'ları ayrı atamaya bölündü), `SPACE_EMPTY_LINE`/`TOO_MANY_ARGS`/
      `SPC_BEFORE_NL`/`NO_SPC_AFR_PAR`/`NEWLINE_PRECEDES_FUNC`/`EMPTY_LINE_EOF`
      (1'er — `expand_value.c`'de `append_plain_chunk`'ın 5→3 argümana
      düşürülmesi dahil, stop karakterleri zaten tek çağrı yerinde sabit
      olduğu için hardcode edildi). WSL'de `-Wall -Wextra -Werror` ile tüm
      proje hatasız derlendi, gerçek `libreadline` ile linklenip birden fazla
      komutla (`echo`, `pwd`, `export`, `env`, pipe, `cd`) çalıştırılıp
      doğrulandı; davranışta değişiklik yok.
...
- [ ] **42 header eksik** — 48 dosyanın 48'inde de yok (dosya sayısı bu
      oturumdaki `TOO_MANY_FUNCS` bölünmesi + yeni `types.h` ile arttı). Her
      `.c`/`.h` dosyasının başına standart 42 header eklenmeli.
...
- [x] **En çok hata veren dosyalar** — bu liste yukarıdaki "42 header ve
      `TOO_MANY_LINES`/`TOO_MANY_VARS_FUNC` hariç kalan TÜM norm hataları"
      maddesiyle **DÜZELTİLDİ**; `exec_path.c` (eski 37), `exec_redir.c` (14),
      `executor.h` (9) ve diğer tüm dosyalar artık `INVALID_HEADER` ve iki
      hariç kategori dışında temiz. Ayrıntı için o maddeye bakılabilir.
